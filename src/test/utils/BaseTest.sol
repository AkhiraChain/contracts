...
abstract contract BaseTest is DSTest, Test {
    ...
    function setUp() public virtual {
        ...
        registry = address(new AkhiraRegistry(forwarder));
        factory = address(new AkhiraFactory(forwarder, registry));
        contractPublisher = address(new ContractPublisher(forwarder));
        AkhiraRegistry(registry).grantRole(AkhiraRegistry(registry).OPERATOR_ROLE(), factory);
        AkhiraRegistry(registry).grantRole(AkhiraRegistry(registry).OPERATOR_ROLE(), contractPublisher);
        fee = address(new AkhiraFee(forwarder, factory));
        AkhiraFactory(factory).addImplementation(address(new TokenERC20(fee)));
        AkhiraFactory(factory).addImplementation(address(new TokenERC721(fee)));
        AkhiraFactory(factory).addImplementation(address(new TokenERC1155(fee)));
        AkhiraFactory(factory).addImplementation(address(new DropERC20(fee)));
        AkhiraFactory(factory).addImplementation(address(new MockContract(bytes32("DropERC721"), 1)));
        AkhiraFactory(factory).addImplementation(address(new DropERC721(fee)));
        AkhiraFactory(factory).addImplementation(address(new MockContract(bytes32("DropERC1155"), 1)));
        AkhiraFactory(factory).addImplementation(address(new DropERC1155(fee)));
        AkhiraFactory(factory).addImplementation(address(new MockContract(bytes32("Marketplace"), 1)));
        AkhiraFactory(factory).addImplementation(address(new Marketplace(address(weth), fee)));
        AkhiraFactory(factory).addImplementation(address(new Split(fee)));
        // AkhiraFactory(factory).addImplementation(address(new Pack(address(0), address(0), fee)));
        AkhiraFactory(factory).addImplementation(address(new Multiwrap(address(weth))));
        AkhiraFactory(factory).addImplementation(address(new VoteERC20()));
        vm.stopPrank();
        ...
        returns (address proxyAddress)
    {
        ...
        proxyAddress = AkhiraFactory(factory).deployProxy(bytes32(bytes(_contractType)), _initializer);
