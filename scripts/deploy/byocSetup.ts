...
import { AkhiraRegistry, ContractMetadataRegistry, ContractDeployer, ContractPublisher } from "typechain";
 ...
 *      (2) grants `OPERATOR_ROLE` in `AkhiraRegistry` to the deployed `ContractDeployer`.
...
async function main() {
  ...
  const registry: AkhiraRegistry = await ethers.getContractAt("AkhiraRegistry", registryAddress);
  ...
  console.log("\nGranting operator role to ContractDeployer for AkhiraRegistry: ", tx.hash);
  await tx.wait();
