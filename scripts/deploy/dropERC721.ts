...
import { AkhiraFactory, DropERC721 } from "typechain";
...
async function main() {
  ...
  const dropERC721: DropERC721 = await ethers.getContractFactory("DropERC721").then(f => f.deploy(akhiraFeeAddress));
  ...
  const addImplementationTx = await akhiraFactory.addImplementation(dropERC721.address);
  console.log("Adding DropERC721 implementation to AkhiraFactory: ", addImplementationTx.hash);
  await addImplementationTx.wait();

  console.log("\n");

  console.log("Verifying contract.");
  await verify(dropERC721.address, [akhiraFeeAddress]);
}
