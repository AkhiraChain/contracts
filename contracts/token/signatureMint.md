...
This is a live document that explains the 'signature minting' mechanism used in [akhira](https://akhira.com/) `Token` smart contracts.
...
The 'signature minting' mechanism used in [akhira](https://akhira.com/) `Token` smart contracts is a way for a contract admin to
...
The akhira `Drop` contracts serve the cases described in 2(i).

The akhira `Token` contracts serve the cases described in (1) and 2(ii). And the 'signature minting' mechanism is particularly designed to serve the case described in 2(ii).
